...
class Developer(commands.Cog):
    ...
    @developer.group(pass_context=True, name="addenv", help="Adds an environment variable.")
    async def developer_addenv(self, ctx, key, *, value):
        ...
        if key.upper() in [var_key.upper() for var_key in current_vars.keys()]:
            current_value = [
                var_key.upper()
                for var_key in current_vars.keys()
                if var_key.upper() == key.upper()
            ][0]
            if "TOKEN" in key.upper() or "KEY" in key.upper():
                current_value = "Token/Key Censored"
            await ctx.send(
                "",
                embed=generate_embed(
                    title="Environment Variable Already Exists",
                    fields={"Key": key, "Value": current_value},
                    color=discord.Color(0x8B0000),
                    bot=self.bot,
                    message=ctx.message
                ),
        ...
        with open(".env", "w") as f:
            for var_key, var_value in current_vars.items():
                f.write(f"{var_key}={var_value}\n")

    @developer.group(
        pass_context=True,
        name="getenv",
        help="Gets the value of an environment variable.",
    )
    async def developer_getenv(self, ctx, key):
        with open(".env", "r") as f:
            current_vars = {
                line.strip().split("=")[0]: line.strip().split("=")[1]
                for line in f.readlines()
        ...
        if key.upper() not in [var_key.upper() for var_key in current_vars.keys()]:
            await ctx.send(
                "",
                embed=generate_embed(
        ...
        current_value = current_vars[current_value]
        if "TOKEN" in key.upper() or "KEY" in key.upper():
            current_value = "Token/Key Censored"
